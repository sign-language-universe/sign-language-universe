 ...
 * 模型：tree_model_v64.onnx（shape_scores 12 + motion_scores 20 + leaf_scores 47 + leaf_exists 47）
 * 缓存：加载版本化文件名（tree_model_v64.*），路径级击穿 VSCode Webview 顽固缓存（见下方注释）
 */
const TreeScorer = (() => {
  ...
  // 查询参数击穿对按路径做键的缓存无效；版本化文件名（tree_model_v64.*）路径唯一，
  // 旧页面引用旧文件名、新页面引用新文件名，路径级缓存不可能跨版本串模型。
  // 注意：后续更新模型时必须同步改这两个文件名（tree_model_v64 → 新版本号）。
  const MODEL_ONNX = 'tree_model_v64.onnx';
  const MODEL_JSON = 'tree_model_v64.json';
